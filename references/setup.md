...
2. Add `Code.gs` and `index.html` from `assets/apps-script-template/`. Use `appsscript.json` when logging is disabled; when logging is enabled, use `appsscript.with-logging.json` but save it in Apps Script under the name `appsscript.json`.
...
7. Open the deployment URL and perform the completion tests listed in `SKILL.md`.
